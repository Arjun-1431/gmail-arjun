...
function asyncRoute(handler) {
  return (req, res, nextHandler) => {
    Promise.resolve(handler(req, res, nextHandler)).catch(nextHandler);
  };
}

console.log(`Preparing Next.js in ${dev ? "development" : "production"} mode...`);
...
app.prepare().then(() => {
  ...
  server.get("/api/auth/google", (req, res, nextHandler) => {
    try {
      const auth = getOAuthClient();
  ...
  server.get(
    ...
    asyncRoute(async (req, res) => {
      if (!req.session.googleTokens) {
        return res.json({ connected: false, emailAddress: null });
      }

      if (req.session.gmailEmail) {
        return res.json({
          connected: true,
          emailAddress: req.session.gmailEmail,
        });
      }
      ...
      const profile = await gmail.users.getProfile({ userId: "me" });
      req.session.gmailEmail = profile.data.emailAddress;
      res.json({ connected: true, emailAddress: profile.data.emailAddress });
    })
  );

  server.post("/api/auth/logout", (req, res) => {
    req.session.destroy(() => {
      res.clearCookie("gmail.sid");
      res.json({ ok: true });
    });
  });
  ...
  server.get(
    ...
    asyncRoute(async (req, res) => {
      const category =
        INBOX_CATEGORY_LABELS[req.query.category] ||
        INBOX_CATEGORY_LABELS.primary;
      res.json(await listMessages(req, ["INBOX", category]));
    })
  );

  server.get(
    "/api/gmail/sent",
    asyncRoute(async (req, res) => res.json(await listMessages(req, ["SENT"])))
  );

  server.get(
    "/api/gmail/unread",
    asyncRoute(async (req, res) => res.json(await listMessages(req, ["UNREAD"])))
  );

  server.get(
    "/api/gmail/drafts",
    asyncRoute(async (req, res) => res.json(await listMessages(req, ["DRAFT"])))
  );

  server.get(
    "/api/gmail/spam",
    asyncRoute(async (req, res) => res.json(await listMessages(req, ["SPAM"])))
  );

  server.get(
    "/api/gmail/trash",
    asyncRoute(async (req, res) => res.json(await listMessages(req, ["TRASH"])))
  );
  ...
  server.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      const [inbox, sent, drafts, unread, spam, trash] = await Promise.all([
        labelCount(gmail, "INBOX"),
        labelCount(gmail, "SENT"),
        labelCount(gmail, "DRAFT"),
        labelCount(gmail, "UNREAD"),
        labelCount(gmail, "SPAM"),
        labelCount(gmail, "TRASH"),
      ]);
  ...
  server.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      res.json(normalizeFullMessage(message.data));
    })
  );

  server.get(
    "/api/gmail/message/:id/attachment/:attachmentId",
    asyncRoute(async (req, res) => {
      const gmail = getAuthedGmail(req);
  ...
  server.post(
    ...
    asyncRoute(async (req, res) => {
      ...
      const sent = await gmail.users.messages.send({
        userId: "me",
        requestBody: {
          raw: createRawEmail(req.body),
        },
      });
      res.status(201).json(sent.data);
    })
  );
  ...
  server.patch(
    ...
    asyncRoute(async (req, res) => {
      ...
      res.json(normalizeMessage(updated.data));
    })
  );
  ...
  server.patch(
    ...
    asyncRoute(async (req, res) => {
      ...
      account.encryptedTokens = encryptTokens(req.session.googleTokens);
      writeJobStore(store);
      res.json({ automationEnabled: account.automationEnabled });
    })
  );

  server.post(
    "/api/jobs/followups/:id/send",
    asyncRoute(async (req, res) => {
      const gmail = getAuthedGmail(req);
      const profile = await gmail.users.getProfile({ userId: "me" });
      const store = readJobStore();
      const account = getAccount(store, profile.data.emailAddress);
      const application = account.applications[req.params.id];
      if (!application) {
      ...
      const sent = await sendFollowUp(gmail, application);
      application.lastFollowUpAt = new Date().toISOString();
      application.followUpCount = (application.followUpCount || 0) + 1;
      account.encryptedTokens = encryptTokens(req.session.googleTokens);
      writeJobStore(store);
      res.status(201).json({ sent, application });
    })
  );
  ...
  server.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      account.encryptedTokens = encryptTokens(req.session.googleTokens);
      writeJobStore(store);
      res.json({ replies: serializeJobReplies(account) });
    })
  );

  server.post(
    "/api/jobs/replies/analyze",
    asyncRoute(async (req, res) => {
      const account = await analyzeJobReplies(req);
      res.json({ replies: serializeJobReplies(account) });
    })
  );

  server.post(
    "/api/jobs/replies/:id/generate",
    asyncRoute(async (req, res) => {
      const gmail = getAuthedGmail(req);
      const profile = await gmail.users.getProfile({ userId: "me" });
      const store = readJobStore();
      const account = getAccount(store, profile.data.emailAddress);
      const jobReply = account.jobReplies[req.params.id];
      if (!jobReply) {
        return res.status(404).json({ error: "Job reply not found." });
      }
      const draft = await generateJobReplyEmail(jobReply);
      res.json(draft);
    })
  );

  server.post(
    "/api/jobs/replies/:id/send",
    asyncRoute(async (req, res) => {
      const gmail = getAuthedGmail(req);
      const profile = await gmail.users.getProfile({ userId: "me" });
      const store = readJobStore();
      ...
      const generated =
        req.body.subject && req.body.bodyHtml
          ...
          : await generateJobReplyEmail(jobReply);
      const sent = await gmail.users.messages.send({
        userId: "me",
        requestBody: {
          threadId: jobReply.threadId,
      ...
      account.encryptedTokens = encryptTokens(req.session.googleTokens);
      writeJobStore(store);
      res.status(201).json({ sent: sent.data, reply: jobReply });
    })
  );
  ...
  server.get(
    ...
    asyncRoute(async (req, res) => {
      ...
      account.encryptedTokens = encryptTokens(req.session.googleTokens);
      writeJobStore(store);
      res.json({ emails: serializeImportantInbox(account) });
    })
  );

  server.post(
    "/api/inbox/important/analyze",
    asyncRoute(async (req, res) => {
      const account = await analyzeImportantInbox(req);
      res.json({ emails: serializeImportantInbox(account) });
    })
  );
  ...
  server.use((error, req, res, nextHandler) => {
    if (res.headersSent) return nextHandler(error);
  ...
  setInterval(() => {
    runDueFollowUps().catch((error) => {
      console.warn(`Follow-up scheduler failed: ${error.message}`);
    });
  }, 15 * 60 * 1000);
}).catch((error) => {
